...
// Penting: Agar cookies/session terbaca saat request
...
// --- FUNGSI ADD TO CART (YANG DIPERBARUI) ---
const addToCart = async () => {
  ...
  try {
    ...
    Swal.fire({
      ...
      showCancelButton: true,
      confirmButtonText: 'Lihat Keranjang', // Tawarkan langsung ke Cart
      cancelButtonText: 'Lanjut Belanja',
      confirmButtonColor: '#000000',
    }).then((result) => {
      if (result.isConfirmed) {
        router.push({ name: 'Cart' }); // Pindah ke halaman Cart
      }
    });

  } catch (error) {
    // 4. Gagal (Misal: Barang sudah ada di keranjang)
      ...
      <section class="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div class="flex flex-col-reverse md:flex-row gap-4">
          <div class="flex md:flex-col gap-3 justify-center md:justify-start overflow-x-auto md:overflow-visible">
            <img v-for="(img, index) in gallery" :key="index" :src="img" class="w-20 h-24 object-cover cursor-pointer rounded-md border-2 transition-all hover:opacity-80" :class="activeImage === img ? 'border-black' : 'border-transparent'" @click="activeImage = img">
          </div>
          <div class="flex-1 bg-gray-100 rounded-2xl flex items-center justify-center p-4 aspect-square overflow-hidden relative">
            <img :src="activeImage" class="w-full h-full object-contain mix-blend-multiply transition-transform duration-500 hover:scale-110">
        ...
        <div>
          ...
          <div class="flex items-center mb-6">
             <div class="flex text-yellow-400">
                <svg v-for="i in 5" :key="i" class="w-5 h-5" :class="i <= product.condition ? 'fill-current' : 'text-gray-300'" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
             </div>
          ...
          <div class="flex items-center gap-4">
            <button 
              @click="addToCart" 
              class="w-full bg-black text-white font-bold py-4 px-6 rounded-xl hover:bg-gray-800 transition-all transform active:scale-95 shadow-lg flex items-center justify-center gap-2"
            >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div v-for="related in relatedProducts" :key="related.id" class="group cursor-pointer" @click="openRelated(related.id)">
            <div class="bg-gray-100 rounded-xl mb-3 overflow-hidden aspect-[4/5] relative">
                <img :src="related.image || 'https://via.placeholder.com/200'" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
            </div>
